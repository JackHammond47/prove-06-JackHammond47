    ...
    private static bool IsAnagram(string word1, string word2) {
        ...
        foreach (char letter in word1)
            if (!anagram1.ContainsKey(letter))
                anagram1.Add(letter, 1);
            else
                anagram1[letter] += 1;
        foreach (char letter in word2)
            if (!anagram2.ContainsKey(letter))
                anagram2.Add(letter, 1);
            else
                anagram2[letter] += 1;
        foreach (var (key, value) in anagram1)
        {
            if (!anagram2.ContainsKey(key))
                return false;
            else if (value != anagram2[key])
                return false;
        }
        foreach (var (key, value) in anagram2)
        {
            if (!anagram1.ContainsKey(key))
                return false;
        }
        return true;
    }
